#!/usr/bin/env python3
"""
Run Aggregated Channel Importance Analysis for All Datasets
============================================================

This script runs the aggregated classification analysis for all 4 datasets:
- MOCAS
- Heat the Chair (HTC)
- N-Back
- WAUC
...
Usage:
    python run_all_datasets.py           # Full run (all datasets)
    python run_all_datasets.py --test    # Test run (quick validation)
"""
...
SCRIPT_NAME = os.path.join(SCRIPT_DIR, 'test_feature_classification_agg.py')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run aggregated channel importance analysis for all datasets'
    )
    ...
    print_header(f"EEG Channel Importance - Aggregated Model Ensemble - ALL DATASETS ({mode_text})")
    ...
    if success_count == len(DATASETS):
        ...
        print("\nResults have been saved to: experiment_results/")
        if test_mode:
            print("\nOutput files (TEST MODE):")
            print("  • channel_importance_aggregated_mocas_classification_TEST_*.csv")
            print("  • channel_importance_aggregated_htc_classification_TEST_*.csv")
            print("  • channel_importance_aggregated_nback_classification_TEST_*.csv")
            print("  • channel_importance_aggregated_wauc_classification_TEST_*.csv")
            print("  • run_details_aggregated_*_classification_TEST_*.csv (detailed metrics)")
            print("\n💡 Test passed! Run without --test for full analysis")
        else:
            print("\nOutput files:")
            print("  • channel_importance_aggregated_mocas_classification_*.csv")
            print("  • channel_importance_aggregated_htc_classification_*.csv")
            print("  • channel_importance_aggregated_nback_classification_*.csv")
            print("  • channel_importance_aggregated_wauc_classification_*.csv")
            print("  • run_details_aggregated_*_classification_*.csv (detailed metrics)")
        return 0
    else:
