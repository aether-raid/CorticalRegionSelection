#!/usr/bin/env python3
"""
Run Aggregated Channel Importance Analysis for All Datasets - REGRESSION WITH LNSO CV
======================================================================================

This script runs the aggregated regression analysis with Leave-N-Subjects-Out cross-validation
for all 4 datasets:
- MOCAS
- Heat the Chair (HTC)
- N-Back
...
Results are saved to the Regression_tests directory.

Usage:
    python run_all_datasets_agg_loso.py           # Full run (all datasets)
    python run_all_datasets_agg_loso.py --test    # Test run (quick validation)
"""
...
SCRIPT_NAME = os.path.join(SCRIPT_DIR, 'test_feature_regression_agg_loso.py')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run aggregated channel importance analysis for all datasets (REGRESSION WITH LNSO CV)'
    )
    ...
    print_header(f"EEG Channel Importance - Aggregated Regression Ensemble with LNSO CV - ALL DATASETS ({mode_text})")
    ...
    if test_mode:
        ...
        print(f"Estimated time: 100-160 minutes total (~25-40 min per dataset)")
        print(f"Note: LNSO CV requires more time than standard cross-validation")
    else:
        print(f"\nEstimated time: 480-720 minutes total (~120-180 min per dataset)")
        print(f"Note: LNSO CV performs subject-independent validation with 5 folds per model")
    ...
    if success_count == len(DATASETS):
        ...
        if test_mode:
            ...
            print("  • channel_importance_aggregated_mocas_regression_loso_TEST_*.csv")
            print("  • channel_importance_aggregated_htc_regression_loso_TEST_*.csv")
            print("  • channel_importance_aggregated_nback_regression_loso_TEST_*.csv")
            print("  • channel_importance_aggregated_wauc_regression_loso_TEST_*.csv")
            print("\n💡 Test passed! Run without --test for full analysis")
            print("   ⚠️ WARNING: Full run takes 8-12 hours with LNSO CV")
        else:
            print("\nOutput files:")
            print("  • channel_importance_aggregated_mocas_regression_loso_*.csv")
            print("  • channel_importance_aggregated_htc_regression_loso_*.csv")
            print("  • channel_importance_aggregated_nback_regression_loso_*.csv")
            print("  • channel_importance_aggregated_wauc_regression_loso_*.csv")
        return 0
    else:
