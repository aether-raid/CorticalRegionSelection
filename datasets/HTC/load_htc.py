...
"""
Heat the Chair (HTC) Dataset Processing Pipeline for EEG Cognitive Workload Analysis
==================================================================================

Complete preprocessing pipeline for the Heat the Chair dataset that transforms raw 
EEG data into processed features suitable for TLX-based regression and classification analysis.

DATASET SOURCE:
    Heat the Chair dataset - EEG recordings during cognitive tasks with NASA-TLX workload ratings
    Contains EEG data from Emotiv EPOC+ headset (14 channels, 128Hz sampling rate)

EXPECTED INPUT STRUCTURE:
    data/heat_the_chair/htc/    # Root directory
        subject_01/
            task1/
                phase1.parquet       # Phase 1 EEG data (not used)
                phase2.parquet       # Phase 2 EEG data (USED for analysis)
                tlx.json            # NASA-TLX scores
                scores.csv          # Additional scores
            task2/
                phase2.parquet
                tlx.json
                scores.csv
...
TLX.JSON STRUCTURE:
    {
        "mental_demand": 0-10,      # Mental workload
        "physical_demand": 0-10,    # Physical workload
        "temporal_demand": 0-10,    # Time pressure
        "performance": 0-10,        # Task performance (inverted: higher = better)
        "effort": 0-10,             # Effort required
        "frustration": 0-10         # Frustration level
    }
    
    Combined TLX Score Calculation:
        • Average of all 6 subscales: (sum of all dimensions) / 6
        • Each subscale is 0-10, so average is 0-10
        • Rescaled to 0-100: (average / 10) * 100
        • Final TLX score range: 0-100
    
    Multi-Label Target Files (NEW):
        • Each TLX subscale gets its own target file with suffix
        • S01_1_features.txt                  - Combined TLX (default, 0-100)
        • S01_1_features_mental.txt           - Mental demand (0-100, rescaled from 0-10)
        • S01_1_features_physical.txt         - Physical demand (0-100, rescaled from 1-10)
        • S01_1_features_temporal.txt         - Temporal demand (0-100, rescaled from 1-10)
        • S01_1_features_performance.txt      - Performance (0-100, rescaled from 1-10)
        • S01_1_features_effort.txt           - Effort (0-100, rescaled from 1-10)
        • S01_1_features_frustration.txt      - Frustration (0-100, rescaled from 1-10)
...
PROCESSING PIPELINE (5 STEPS):
    ...
    STEP 1: EEG File Extraction & Reformatting (Phase 2 only)
        ...
        • Output: data/heat_the_chair/htc_raw_eeg_extracted/
    ...
    STEP 2: Statistical Feature Extraction
        ...
        • Output: data/heat_the_chair/htc_features_extracted/S{X}_{Y}_features.parquet
        
    STEP 3: Combined TLX Target File Creation + Individual Subscales
        • Reads tlx.json scores for all 6 dimensions
        • Calculates combined TLX score (0-100 scale)
        • Creates multiple target files per recording (multi-label format):
            - S{X}_{Y}_features.txt                  (Combined TLX, default)
            - S{X}_{Y}_features_mental.txt           (Mental demand subscale)
            - S{X}_{Y}_features_physical.txt         (Physical demand subscale)
            - S{X}_{Y}_features_temporal.txt         (Temporal demand subscale)
            - S{X}_{Y}_features_performance.txt      (Performance subscale)
            - S{X}_{Y}_features_effort.txt           (Effort subscale)
            - S{X}_{Y}_features_frustration.txt      (Frustration subscale)
        • Output: data/heat_the_chair/htc_features_extracted/S{X}_{Y}_features*.txt
        
    STEP 4: Copy to TLX Regression Datasets
        • Creates 2 regression datasets with combined TLX targets:
            - htc_time_regression/    (Raw EEG + TLX targets)
            - htc_feature_regression/ (Features + TLX targets)
    
    STEP 5: Create TLX Classification Datasets (3-bin classification)
        • Bins combined TLX scores into Low/Medium/High workload classes
        • Uses 33rd and 67th percentiles as bin boundaries
        • Organizes by workload class for classification tasks
        • Creates 2 classification datasets:
            - htc_time_classification/{low,medium,high}/
            - htc_feature_classification/{low,medium,high}/

FINAL OUTPUT STRUCTURE (4 DATASETS TOTAL):
    data/heat_the_chair/
        htc_raw_eeg_extracted/              # Extracted phase2 with decomposition
            S01_1_eeg_raw.parquet
            S01_2_eeg_raw.parquet
            ...
        htc_features_extracted/             # Statistical features + targets
            S01_1_features.parquet
            S01_1_features.txt                 # Combined TLX score (0-100, default)
            S01_1_features_mental.txt          # Mental demand (0-100, rescaled)
            S01_1_features_physical.txt        # Physical demand (0-100, rescaled)
            S01_1_features_temporal.txt        # Temporal demand (0-100, rescaled)
            S01_1_features_performance.txt     # Performance (0-100, rescaled)
            S01_1_features_effort.txt          # Effort (0-100, rescaled)
            S01_1_features_frustration.txt     # Frustration (0-100, rescaled)
            S01_2_features.parquet
            S01_2_features.txt
            S01_2_features_mental.txt
            S01_2_features_physical.txt
            ...
        htc_time_regression/               # Dataset 1: Raw EEG regression
            S01_1_eeg_raw.parquet
            S01_1_eeg_raw.txt                  # Combined TLX (default)
            S01_1_eeg_raw_mental.txt           # Mental demand subscale
            S01_1_eeg_raw_physical.txt         # Physical demand subscale
            ...
        htc_feature_regression/            # Dataset 2: Feature regression
            S01_1_features.parquet
            S01_1_features.txt                 # Combined TLX (default)
            S01_1_features_mental.txt          # Mental demand subscale
            S01_1_features_physical.txt        # Physical demand subscale
            ...
        htc_time_classification/           # Dataset 3: Raw EEG classification
            low/
                S01_1_eeg_raw.parquet
                ...
            medium/
            ...
            high/
                ...
                ...
        htc_feature_classification/        # Dataset 4: Feature classification
            low/
                S01_1_features.parquet
                ...
            medium/
...
INTEGRATION:
    ...
    • EEGRawRegressionDataset with target_suffix parameter for multi-label regression
    • TimeRawRegressionDataset with target_suffix parameter for multi-label regression
    • Standard regression and classification pipelines
    • Channel importance analysis and feature selection methods
    
    Example Usage:
        # Load combined TLX (default behavior)
        from core.regression_datasets import EEGRawRegressionDataset
        dataset = EEGRawRegressionDataset(data_dir="data/heat_the_chair/htc_feature_regression")
        
        # Load specific TLX subscale (mental demand)
        dataset = EEGRawRegressionDataset(
            data_dir="data/heat_the_chair/htc_feature_regression",
            target_suffix="mental"
        )
        
        # Load different subscale (effort)
        dataset = EEGRawRegressionDataset(
            data_dir="data/heat_the_chair/htc_feature_regression",
            target_suffix="effort"
        )

USAGE:
    python datasets/HTC/load_htc.py
    
    Or call individual functions for specific TLX dimensions:
    • reformat_raw_eeg()
    • extract_features_from_htc_files()
...
data_path = "data/heat_the_chair/htc"  # Source data
raw_output_path = "data/heat_the_chair/htc_raw_eeg_extracted"  # Extracted raw EEG
features_output_path = "data/heat_the_chair/htc_features_extracted"  # Statistical features
time_regression_path = "data/heat_the_chair/htc_time_regression"  # Time regression dataset
feature_regression_path = "data/heat_the_chair/htc_feature_regression"  # Feature regression dataset
time_classification_path = "data/heat_the_chair/htc_time_classification"  # Time classification dataset
feature_classification_path = "data/heat_the_chair/htc_feature_classification"  # Feature classification dataset
...
# Mapping from JSON keys (with _demand suffix) to output file suffixes (without suffix)
...
def reformat_raw_eeg(source_dir, output_dir, sampling_rate=128.0, raw_only=False):
    """
    Extract and reformat phase2 EEG files from HTC dataset.
    ...
    Args:
        source_dir (str): Path to htc/ directory containing subject folders
    ...
    print(f"\n{'='*60}")
    if raw_only:
        print(f"HTC EEG EXTRACTION - RAW ONLY (PHASE 2 ONLY)")
    else:
        print(f"HTC EEG EXTRACTION WITH SUBBAND DECOMPOSITION (PHASE 2 ONLY)")
    print(f"{'='*60}")
...
def _process_single_eeg_file(eeg_file, original_path, output_path, sampling_rate=128.0):
    """
    Helper function to process a single EEG file for parallel execution.
    
    Args:
        eeg_file (Path): Path to the extracted EEG file
        original_path (Path): Path to original htc/ directory
        output_path (Path): Output directory for features
...
def extract_features_from_htc_files(input_dir, output_dir, original_data_dir, sampling_rate=128.0, n_jobs=None):
    """
    Extract statistical features from HTC EEG files using parallel processing.
    ...
    Uses multiprocessing to parallelize feature extraction across multiple CPU cores.
    ...
    Args:
        ...
        original_data_dir (str): Path to original htc/ directory with phase2.parquet files
        sampling_rate (float): Sampling frequency in Hz (default: 128.0)
        n_jobs (int): Number of parallel jobs. None = all available CPUs, 1 = sequential
    ...
    print(f"\n{'='*60}")
    print(f"HTC FEATURE EXTRACTION (PARALLEL VERSION)")
    print(f"{'='*60}")
    ...
    eeg_files = sorted(list(input_path.glob("S*_*_eeg_raw.parquet")))
    ...
    if n_workers > 1:
        print(f"Processing files in parallel using {n_workers} workers...")
        
        # Create a partial function with fixed arguments
        process_func = partial(
            _process_single_eeg_file,
        ...
        for eeg_file in tqdm(eeg_files, desc="Extracting features"):
            result = _process_single_eeg_file(
                eeg_file,
                original_path=original_path,
                output_path=output_path,
                sampling_rate=sampling_rate
    ...
    processed_files = sum(1 for f in file_info if f['status'] == 'success')
...
def create_target_files_from_tlx(source_dir, features_dir):
    """
    Create multi-label TLX target files for each EEG recording.
    
    STEP 3: Reads tlx.json files and creates multiple target files per recording.
    
    Multi-Label Format:
        For each recording (e.g., S01_1_features.parquet), creates:
        1. S01_1_features.txt                  - Combined TLX (0-100, default)
        2. S01_1_features_mental.txt           - Mental demand (0-100, rescaled from 1-10)
        3. S01_1_features_physical.txt         - Physical demand (0-100, rescaled from 1-10)
        4. S01_1_features_temporal.txt         - Temporal demand (0-100, rescaled from 1-10)
        5. S01_1_features_performance.txt      - Performance (0-100, rescaled from 1-10)
        6. S01_1_features_effort.txt           - Effort (0-100, rescaled from 1-10)
        7. S01_1_features_frustration.txt      - Frustration (0-100, rescaled from 1-10)
    
    Combined TLX Calculation:
        1. Read all 6 TLX subscales (each 1-10 from raw data)
        2. Calculate average: sum(all_dimensions) / 6
        3. Rescale to 0-100: ((average - 1) / 9) * 100
        4. Save as default target file (no suffix)
    
    Usage with EEGRawRegressionDataset:
        # Load combined TLX (default)
        dataset = EEGRawRegressionDataset(data_dir="htc_feature_regression")
        
        # Load mental demand subscale
        dataset = EEGRawRegressionDataset(data_dir="htc_feature_regression", 
                                         target_suffix="mental")
        
        # Load effort subscale
        dataset = EEGRawRegressionDataset(data_dir="htc_feature_regression", 
                                         target_suffix="effort")
    
    Args:
        source_dir (str): Path to htc/ directory containing tlx.json files
    ...
    print(f"\n{'='*60}")
    print(f"CREATING MULTI-LABEL TLX TARGET FILES")
    print(f"{'='*60}")
    print(f"Source: {source_path}")
    print(f"Output: {features_path}")
    print(f"Format: Combined TLX + 6 individual subscales per recording")
    print(f"{'='*60}\n")
    ...
    for subject_folder in sorted(subject_folders):
        ...
        for task_folder in sorted(task_folders):
            ...
            try:
                ...
                # Check all dimensions are present (check JSON keys)
                ...
                # Step 1: Get all 6 subscale scores (should be 0-10 scale each)
                ...
                combined_score = (average_score / 10.0) * 100.0
                ...
                for dim in TLX_DIMENSIONS:
                    json_key = TLX_JSON_KEY_MAPPING[dim]
                    subscale_score_raw = tlx_data[json_key]  # Expected: 0-10 scale
                    
                    # Clamp to valid range (0-10)
                    subscale_score_raw = max(0.0, min(10.0, subscale_score_raw))
                    
                    # Rescale 0-10 → 0-100
                    subscale_score_rescaled = (subscale_score_raw / 10.0) * 100.0
    ...
    # Save summary
    summary = {
        'created_recordings': created_targets,
        'created_target_files': created_targets * 7,
...
def cleanup_parquet_files_without_targets(features_dir):
    """
    Remove feature files that don't have corresponding target files.
    
    STEP 4: Ensures data consistency by removing unpaired files.
    
    Checks for default combined TLX target file (*.txt without suffix).
    ...
    for feature_file in feature_files:
        # Get corresponding default target file name (combined TLX, no suffix)
        base_name = feature_file.stem  # S01_1_features
        target_file = features_path / f"{base_name}.txt"
...
def create_regression_datasets(raw_dir, features_dir, time_output_dir, feature_output_dir):
    """
    Create regression datasets with multi-label TLX targets.
    
    STEP 4: Copies paired raw EEG/features + all TLX target files into regression directories.
    
    For each recording, copies:
        - Feature/raw EEG file
        - Combined TLX target (default, no suffix)
        - All 6 individual subscale targets (with suffixes)
    ...
    # Get all default (combined) TLX target files
    ...
    for target_file in sorted(target_files):
        ...
        # Extract subject and task number
        parts = base_name.replace('_features', '').split('_')
        if len(parts) < 2:
            continue
        
        subject_task = '_'.join(parts)  # S01_1
        ...
        # Copy raw EEG + all targets to time regression dataset
        if raw_file.exists():
            shutil.copy2(raw_file, time_output / raw_file.name)
            
            # Copy combined TLX target (no suffix)
            target_base_raw = f"{subject_task}_eeg_raw"
            shutil.copy2(target_file, time_output / f"{target_base_raw}.txt")
            
            # Copy individual subscale targets
            for dim in TLX_DIMENSIONS:
                subscale_target = features_path / f"{base_name}_{dim}.txt"
                if subscale_target.exists():
                    shutil.copy2(subscale_target, time_output / f"{target_base_raw}_{dim}.txt")
            
            copied_raw += 1
        
        # Copy features + all targets to feature regression dataset
        if feature_file.exists():
            ...
            for dim in TLX_DIMENSIONS:
                subscale_target = features_path / f"{base_name}_{dim}.txt"
                if subscale_target.exists():
                    shutil.copy2(subscale_target, feature_output / f"{base_name}_{dim}.txt")
    ...
    print(f"{'='*60}")
    print(f"Recordings processed: {copied_targets}")
    ...
    print(f"Total target files copied: {copied_targets * 7}")
    
    return {
        'copied_raw': copied_raw,
        'copied_features': copied_features,
        'copied_recordings': copied_targets,
        'copied_target_files': copied_targets * 7,
...
def create_classification_datasets(raw_dir, features_dir, time_output_dir, feature_output_dir):
    ...
    return {
        ...
        'metadata_file': 'classification_metadata.json'
    }


def run_full_pipeline():
    """
    Run the complete HTC preprocessing pipeline with multi-label regression support.
    
    Steps:
    1. Extract and reformat phase2 EEG files (with subband decomposition)
    2. Extract statistical features from ORIGINAL raw data
    3. Create multi-label TLX target files:
       - Combined TLX (0-100 scale, default/no suffix)
       - Individual subscales (0-20 scale, with suffix)
    4. Create regression datasets (2 datasets: time + features, with multi-label targets)
    5. Create classification datasets (2 datasets: time + features, binned into low/medium/high)
    
    Multi-Label Target Files Created:
        For each recording (e.g., S01_1):
        - S01_1_features.txt                  (Combined TLX, 0-100)
        - S01_1_features_mental.txt           (Mental demand, 0-20)
        - S01_1_features_physical.txt         (Physical demand, 0-20)
        - S01_1_features_temporal.txt         (Temporal demand, 0-20)
        - S01_1_features_performance.txt      (Performance, 0-20)
        - S01_1_features_effort.txt           (Effort, 0-20)
        - S01_1_features_frustration.txt      (Frustration, 0-20)
    ...
    Final Output: 4 datasets total
        - htc_time_regression/       (7 target files per recording)
        - htc_feature_regression/    (7 target files per recording)
        - htc_time_classification/   (uses combined TLX for binning)
        - htc_feature_classification/ (uses combined TLX for binning)
    """
    
    print(f"\n{'#'*60}")
    print(f"# HTC DATASET PREPROCESSING PIPELINE")
    print(f"# Multi-Label Regression Format")
    print(f"# Combined TLX (0-100) + 6 Subscales (0-20)")
    ...
    # STEP 3: Create multi-label TLX target files
    print(f"\n{'='*60}")
    print(f"STEP 3: CREATING MULTI-LABEL TLX TARGET FILES")
    print(f"{'='*60}")
    ...
    print(f"#      - Uses combined TLX for low/medium/high binning")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='HTC (Heat the Chair) Dataset Processing Pipeline for EEG Cognitive Workload Analysis',
...
Steps:
  ...
  5   - Create classification datasets
    ...
    if args.raw_only:
        ...
        print(f"\n{'#'*60}")
        print(f"# HTC RAW EEG EXTRACTION (NO PROCESSING)")
        ...
        print(f"\n{'#'*60}")
        print(f"# HTC TIME-SERIES ONLY PIPELINE")
        print(f"{'#'*60}\n")
        reformat_raw_eeg(data_path, raw_output_path, sampling_rate=128.0)
        ...
        # Feature-only mode
        print(f"\n{'#'*60}")
        print(f"# HTC FEATURE-ONLY PIPELINE")
        print(f"{'#'*60}\n")
        reformat_raw_eeg(data_path, raw_output_path, sampling_rate=128.0)
        extract_features_from_htc_files(raw_output_path, features_output_path, original_data_dir=data_path, sampling_rate=128.0)
        create_target_files_from_tlx(data_path, features_output_path)
        cleanup_parquet_files_without_targets(features_output_path)
        create_regression_datasets(raw_output_path, features_output_path, time_regression_path, feature_regression_path)
        create_classification_datasets(raw_output_path, features_output_path, time_classification_path, feature_classification_path)
